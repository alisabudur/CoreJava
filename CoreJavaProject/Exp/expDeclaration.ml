...
type exp2 = NewExp of cJClassType*varList
...
let rec toStringExp2 (e: exp2) =
	match e with
	| NewExp(classType, varList) -> String.concat "" ["new "; (toStringCJClassType classType); "("; toStringVarList(varList); ")"]
	| MethodCall(varName, name, varList) -> String.concat "" [varName; "."; name; "("; toStringVarList(varList); ")"];;
